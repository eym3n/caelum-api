...
class DataSignals(BaseModel):
    """Data-driven rationale extracted from campaign and experiment datasets."""

    performance_overview: str = Field(
        default="Summarize key KPIs (conversion rate, bounce, engagement, scroll depth, spend) with supporting campaign IDs.",
        description="High-level quantitative summary with referenced campaign IDs and metrics.",
    )
    audience_behavior: str = Field(
        default="Discuss traffic sources, device mix, and channel behaviors using concrete percentages.",
        description="Observations about traffic mix, device shares, and audience behavior derived from the dataset.",
    )
    design_opportunities: str = Field(
        default="Translate data into opportunities for layout, CTA placement, content emphasis, and creative direction.",
        description="Data-backed opportunities that should shape hero layout, CTA prominence, storytelling emphasis, etc.",
    )
    risks_and_mitigations: str = Field(
        default="Call out risks (e.g., mobile complaints, dataset gaps) and how the design will mitigate them.",
        description="Cautions derived from the data along with mitigation tactics baked into the design blueprint.",
    )
    experiment_directives: str = Field(
        default="Document experiment learnings that must be preserved (e.g., hero form uplift, mobile adjustments).",
        description="Directives coming from experimentation (form placement, interaction timing, progressive disclosure).",
    )
...
class DesignGuidelines(BaseModel):
    ...
    data_signals: DataSignals = Field(
        default_factory=DataSignals,
        description="Data-driven reasoning derived from uploaded campaign/experiment datasets.",
    )
