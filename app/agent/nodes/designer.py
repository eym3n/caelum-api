...
def designer(state: BuilderState) -> BuilderState:
    ...
    if has_guidelines:
        ...
        guidelines_json = json.dumps(design_guidelines, indent=2)
        prompt = (
            f"""
...
4. **Workflow:**
   - Use `designer_batch_create_files` to create globals.css and tailwind.config.ts
   - Use `designer_batch_update_files` or `designer_batch_update_lines` to update layout.tsx
   - Run `lint_project` to validate
   - Fix any errors

5. **Final Response:**
   - Output "DONE" when you are finished, nothing else.
