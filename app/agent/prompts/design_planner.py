...
5. **Theme:** Respect `branding.theme` (light vs dark). Provide guidance on how colors should feel, but NOT as CSS tokens—describe the story (e.g., “deep plum base with neon coral accents”). Light themes must explicitly call for ink-rich typography (deep nav/heading/body colors, ≥4.5:1 contrast), anchored buttons, and nav link treatments that stay legible over hero imagery or translucent nav shells. **Call out specific Tailwind class stacks (text-slate-900 / bg-slate-100 / border-slate-300 etc.) the coder must use to guarantee ≥4.5:1 contrast for primary, secondary, and ghost buttons plus nav links. Stress that in light themes all ghost/outline buttons sit on light surfaces—give them visible borders/background tints (e.g., `border-slate-400 bg-white/60`) and never allow primary fills to approach white (keep accent hues saturated).**
...
- In light themes, every blueprint must spell out exact color tokens for nav links and button tiers (primary/secondary/ghost) to ensure ≥4.5:1 contrast against the surfaces they sit on. Explicitly warn the coder if any combination risks low readability and suggest fallback class stacks.
