...
- In light themes, explicitly implement the blueprint’s contrast instructions: nav links must use deep ink colors (e.g., `text-slate-900`) on neutral backgrounds, primary buttons require saturated fills (no near-white tones) with white text, secondary buttons need sufficiently opaque borders/fills, and ghost/outline buttons must add a visible border/tint (e.g., `border-slate-400 bg-white/60`) so they never blend into the surface. Never fall back to low-contrast grayscale or semi-transparent white.
